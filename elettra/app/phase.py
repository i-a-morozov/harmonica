...
from harmonica.util import LIMIT, bpm_select, mod, pv_make
...
def _compute_phase(
    *,
    ...
    pair = None
    if advance:
        total = float(cs.get(f"{prefix}:TAIL:MODEL:F{target}"))
        model = torch.tensor([cs.get(f"{prefix}:{name}:MODEL:F{target}") for name in bpm], dtype=dtype, device=device)
        model, _ = Decomposition.phase_adjacent(total / (2.0 * np.pi), model)
        model_numpy = model.cpu().numpy()
        names = [*bpm.keys()]
        pair = [f"{names[i]}-{names[i + 1]}" for i in range(len(bpm) - 1)]
        pair.append(f"{names[-1]}-{names[0]}")
    ...
    if not use_dht:
        ...
        table = dht.angle()
        table -= 2.0 * np.pi * frequency_value * torch.linspace(
            0, sample_length - 1, sample_length, dtype=dtype, device=device
        )
        table = mod(table, 2.0 * np.pi, -np.pi)
    ...
    if table is not None:
        ...
        table_df = pd.concat(
            [
                pd.DataFrame({"STEP": range(1, step + 1), "BPM": name, "PHASE": table[i].cpu().numpy()})
                for i, name in enumerate(bpm)
            ],
            ignore_index=True,
        )

    summary_df = pd.DataFrame({"BPM": [*bpm.keys()], "PHASE": output, "ERROR": error_output})
    ...
    if monotonic:
        ...
        for i in range(1, len(phase_table)):
            while phase_table[i] < phase_table[i - 1]:
                phase_table[i] += 2.0 * np.pi
        tune_integer = int(round(phase_table[-1] / (2.0 * np.pi)))
        monotonic_df = pd.DataFrame({"BPM": [*bpm.keys()], "PHASE": phase_table})

    advance_df = None
    advance_error_df = None
    tune_model = None
    tune_phase = None
    rms_error = None
    if advance and model_numpy is not None and pair is not None:
        total = float(cs.get(f"{prefix}:FREQUENCY:VALUE:{target}"))
        phase_adj, sigma_adj = Decomposition.phase_adjacent(total, value, sigma_phase=error)
        phase_adj_numpy = phase_adj.cpu().numpy()
        sigma_adj_numpy = sigma_adj.cpu().numpy()
        tune_model = float(np.sum(model_numpy) / (2.0 * np.pi))
        tune_phase = float(np.sum(phase_adj_numpy) / (2.0 * np.pi))
        advance_df = pd.concat(
            [
                pd.DataFrame({"CASE": "MODEL", "PAIR": pair, "ADVANCE": model_numpy, "SIGMA": np.zeros_like(sigma_adj_numpy)}),
                pd.DataFrame({"CASE": "PHASE", "PAIR": pair, "ADVANCE": phase_adj_numpy, "SIGMA": sigma_adj_numpy}),
            ],
            ignore_index=True,
        )
        advance_error = 100.0 * (model_numpy - phase_adj_numpy) / model_numpy
        sigma_error = 100.0 * sigma_adj_numpy / model_numpy
        rms_error = float(np.sqrt(np.mean(advance_error**2)))
        advance_error_df = pd.DataFrame({"PAIR": pair, "ERROR": advance_error, "SIGMA": sigma_error})

    saved_file = None
    if save:
        timestamp = datetime.now().strftime("%d.%m.%Y_%H-%M-%S")
        mode = "phase_coupled" if coupled else "phase"
    ...
    if update and coupled:
        raise RuntimeError("Update with coupled mode is not supported for phase data.")

    if update and not coupled:
        for name, phase_value, sigma in zip(bpm, output, error_output):
            cs.set(f"{prefix}:{name}:PHASE:VALUE:{plane_up}", float(phase_value))
            cs.set(f"{prefix}:{name}:PHASE:ERROR:{plane_up}", float(sigma))
    ...
    return {
        ...
        "tune_integer": tune_integer,
        "advance_df": advance_df,
        "advance_error_df": advance_error_df,
...
def main() -> None:
    st.set_page_config(page_title="Phase", layout="wide")
    st.title("Phase")

    try:
        app_config: AppConfig = load_app_config("phase")
    ...
    with st.sidebar.form("phase_form", enter_to_submit=False):
        st.caption("Planes")
        ...
        with st.expander("Plane X options", expanded=False):
            ...
            shift_count_x = ui.number_input("Shift sample count (X)", min_value=1, value=256, step=1)
            ...
            monotonic_x = ui.checkbox("Plot monotonic phase (X)", value=False)
            advance_x = ui.checkbox("Plot adjacent advance (X)", value=False)
        ...
        with st.expander("Plane Y options", expanded=False):
            ...
            shift_count_y = ui.number_input("Shift sample count (Y)", min_value=1, value=256, step=1)
            ...
            monotonic_y = ui.checkbox("Plot monotonic phase (Y)", value=False)
            advance_y = ui.checkbox("Plot adjacent advance (Y)", value=False)
            coupled_y = ui.checkbox("Use coupled frequency (Y->X)", value=False)

        plot = ui.checkbox("Show phase plots", value=True)
    ...
    with right_col:
        ...
        plane_options: dict[str, dict[str, object]] = {
            ...
            "y": {
                ...
                "plot": plot,
                "advance": advance_y,
                "monotonic": monotonic_y,
                "coupled": coupled_y,
            },
        }

        results: dict[str, dict[str, object]] = {}
        with st.spinner("Running phase computation..."):
            try:
                for plane in planes:
                    option = plane_options[plane]
                    results[plane] = _compute_phase(
                        plane=plane,
                        ...
                        plot=bool(option["plot"]),
                        advance=bool(option["advance"]),
                        monotonic=bool(option["monotonic"]),
        ...
        for plane in planes:
            ...
            if result["saved_file"] is not None:
                st.write(f"{plane.upper()} phase saved: `{result['saved_file']}`")
        ...
        if plot:
            ...
            for plane in planes:
                ...
                sample_plot = px.scatter(
                    ...
                    y="PHASE",
                    color="BPM",
                    title=f"{result['time']}: PHASE ({result['mode_text']})",
                    opacity=0.75,
                    marginal_y="box",
                )
            ...
            st.subheader("Monotonic")
            for plane in planes:
                result = results[plane]
                if result["monotonic_df"] is None:
                    continue
                st.caption(f"Plane {plane.upper()} ({result['mode_text']}), integer={result['tune_integer']}")
                mono_plot = px.scatter(
                    result["monotonic_df"],
                    x="BPM",
                    y="PHASE",
                    title=f"{result['time']}: MONOTONIC PHASE ({result['mode_text']})",
                    hover_data=["BPM", "PHASE"],
                )
                mono_plot.update_traces(mode="lines+markers", line={"width": 1.5}, marker={"size": 5})
                st.plotly_chart(mono_plot, use_container_width=True)

            st.subheader("Advance")
            for plane in planes:
                result = results[plane]
                if result["advance_df"] is None:
                    continue
                st.caption(
                    f"Plane {plane.upper()} ({result['mode_text']}), "
                    f"model={result['tune_model']:.9f}, phase={result['tune_phase']:.9f}"
                )
                adv_plot = px.scatter(
                    result["advance_df"],
                    x="PAIR",
                    ...
                    color="CASE",
                    error_y="SIGMA",
                    title=f"{result['time']}: ADJACENT ADVANCE ({result['mode_text']})",
                    color_discrete_sequence=["blue", "red"],
                )
                adv_plot.update_traces(marker={"size": 5})
                st.plotly_chart(adv_plot, use_container_width=True)

            st.subheader("Advance Error")
            for plane in planes:
                result = results[plane]
                if result["advance_error_df"] is None:
                    continue
                st.caption(f"Plane {plane.upper()} ({result['mode_text']}), rms={result['rms_error']:.6g}%")
                err_plot = px.scatter(
                    result["advance_error_df"],
                    x="PAIR",
                    y="ERROR",
                    error_y="SIGMA",
                    title=f"{result['time']}: ADJACENT ADVANCE ERROR ({result['mode_text']})",
                    color_discrete_sequence=["blue"],
                )
                err_plot.update_traces(marker={"size": 5})
                st.plotly_chart(err_plot, use_container_width=True)
        else:
            st.subheader("Phase Tables")
            for plane in planes:
