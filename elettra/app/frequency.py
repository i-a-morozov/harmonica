...
from harmonica.statistics import biweight_midvariance, median, standardize, weighted_mean, weighted_variance
...
def _compute_frequency(
    *,
    ...
    if process == "noise":
        flt = Filter(tbt)
        _, noise = flt.estimate_noise(limit=limit)
    ...
    freq(method, f_range=(f_min, f_max))

    if clean:
        data = standardize(freq.frequency, center_estimator=median, spread_estimator=biweight_midvariance)
        factor_tensor = torch.tensor(factor, dtype=dtype, device=device)
        mask = threshold(data, -factor_tensor, +factor_tensor).squeeze(0)
    else:
        mask = torch.ones_like(freq.frequency)

    if process == "none":
        weight = mask.to(dtype)
        center = weighted_mean(freq.frequency, weight=weight)
        spread = weighted_variance(freq.frequency, weight=weight, center=center).sqrt()
    elif process == "noise":
        if noise is None:
            raise RuntimeError("Noise data is unavailable for process='noise'.")
        factor_tensor = torch.stack(
            [spectrum[index] for index, spectrum in zip(freq.ffrft_bin.to(torch.int64), freq.ffrft_spectrum)]
        ) ** 2
        weight = mask * factor_tensor / noise**2
    ...
    output = freq.frequency.cpu().numpy()
    center_value = float(center.cpu().numpy())
    spread_value = float(spread.cpu().numpy())

    if flip:
        output = 1.0 - output
        center_value = 1.0 - center_value

    mask_numpy = mask.to(torch.bool).logical_not().cpu().numpy()
    if process == "none":
        weight_numpy = (~mask_numpy).astype(np.float64)
    else:
    ...
    if save:
        ...
        saved_file = Path(f"frequency_plane_{plane}_length_{length}_time_({timestamp}).npy")
        np.save(saved_file, output)

    if update:
        target = plane.upper()
        cs.set(f"{prefix}:FREQUENCY:VALUE:{target}", center_value)
        cs.set(f"{prefix}:FREQUENCY:ERROR:{target}", spread_value)
        for name, frequency in zip(bpm, output):
            cs.set(f"{prefix}:{name}:FREQUENCY:VALUE:{target}", float(frequency))
            cs.set(f"{prefix}:{name}:FREQUENCY:ERROR:{target}", 0.0)
...
def main() -> None:
    ...
    try:
        app_config: AppConfig = load_app_config("frequency")
    ...
    with st.sidebar.form("frequency_form", enter_to_submit=False):
        st.caption("Planes")
        ...
        with st.expander("Plane X options", expanded=False):
            length_x = ui.number_input("Length (X)", min_value=1, max_value=LIMIT, value=1024, step=1)
        ...
        with st.expander("Plane Y options", expanded=False):
            length_y = ui.number_input("Length (Y)", min_value=1, max_value=LIMIT, value=1024, step=1)
    ...
    with right_col:
        ...
        with st.spinner("Running frequency computation..."):
            try:
                for plane in planes:
                    option = plane_options[plane]
                    results[plane] = _compute_frequency(
                        plane=plane,
        ...
        c3.metric("Frequency points", value=int(first["size"]))
        ...
        if plot:
            ...
            for plane in planes:
                ...
                freq_plot = px.scatter(
                    result["dataframe"],
                    x="BPM",
                    y="FREQUENCY",
                    color="FLAGGED",
                    color_discrete_map={False: "blue", True: "red"},
                    hover_data=["BPM", "FREQUENCY", "WEIGHT"],
                    title=(
                        f"{result['time']}: FREQUENCY "
                        f"({result['output_mean']:.9f}, {result['output_std']:.9f})"
                    ),
                    opacity=0.75,
                    marginal_y="box",
                    category_orders={"BPM": result["dataframe"]["BPM"].tolist()},
                )
                freq_plot.add_hline(result["center"] - result["spread"], line_color="black", line_dash="dash", line_width=1.0)
                freq_plot.add_hline(result["center"], line_color="black", line_dash="dash", line_width=1.0)
                freq_plot.add_hline(result["center"] + result["spread"], line_color="black", line_dash="dash", line_width=1.0)
                freq_plot.update_traces(marker={"size": 10})
