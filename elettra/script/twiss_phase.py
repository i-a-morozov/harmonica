#!/bin/env python

# Import
import sys
import argparse
import os
import numpy
import torch
from datetime import datetime
from harmonica.cs import factory
from harmonica.model import Model
from harmonica.table import Table
from harmonica.twiss import Twiss

# Input arguments flag
_, *last = sys.argv

# Parse arguments
parser = argparse.ArgumentParser(prog='twiss_phase', description='Compute and plot twiss from phase for x and y planes')
parser.add_argument('-m', '--model', type=str, help='model config path', default='elettra.yaml')
parser.add_argument('-l', '--limit', type=int, help='range limit', default=4)
parser.add_argument('--unit', choices=('m', 'mm', 'mk'), help='amplitude units', default='m')
parser.add_argument('--clean', action='store_true', help='flag to clean data')
parser.add_argument('--factor', type=float, help='threshold factor', default=5.0)
parser.add_argument('--plot', action='store_true', help='flag to plot data')
parser.add_argument('--amplitude', action='store_true', help='flag to plot twiss from amplitude data')
parser.add_argument('--phase', action='store_true', help='flag to plot phase advance data')
parser.add_argument('--position', action='store_true', help='flag to use BPM position on x-axis')
parser.add_argument('--prefix', type=str, help='PV prefix', default='BPM')
parser.add_argument('--data', type=str, help='PV data prefix', default='')
parser.add_argument('--tango', action='store_true', help='flag to use tango CS')
parser.add_argument('--device', choices=('cpu', 'cuda'), help='data device', default='cpu')
parser.add_argument('--dtype', choices=('float32', 'float64'), help='data type', default='float64')
parser.add_argument('-u', '--update', action='store_true', help='flag to update harmonica PV')
parser.add_argument('--verbose', action='store_true', help='verbose flag')
args = parser.parse_args(args=None if last else ['--help'])

# Time
TIME = datetime.now().strftime('%d.%m.%Y %H:%M:%S')
if args.verbose:
  print(f'Time: {TIME}')

# Check and set device & data type
dtype = {'float32': torch.float32, 'float64': torch.float64}[args.dtype]
device = args.device
if device == 'cuda' and not torch.cuda.is_available():
  exit('error: CUDA is not available')

# CS
cs = factory(target=('tango' if args.tango else 'epics'))

# Amplitude factor
factor = {'m':1.0E+0, 'mm':1.0E-2, 'mk':1.0E-6}[args.unit]
prefix = args.prefix if not args.data else args.data

# Load monitor data
name = cs.get(f'{args.prefix}:MONITOR:LIST')[:cs.get(f'{args.prefix}:MONITOR:COUNT')]
flag = torch.tensor([cs.get(f'{args.prefix}:{name}:FLAG') for name in name], dtype=torch.int64, device=device)
position = numpy.array([cs.get(f'{args.prefix}:{name}:TIME') for name in name], dtype=numpy.float64)
selected = {bpm: int(mark) for bpm, mark in zip(name, flag.cpu().numpy()) if mark == 1}
if args.verbose:
  print('Monitor list:')
  for key, value in selected.items():
    print(f'{key}: {value}')

# Load x frequency data
value_nux = torch.tensor(cs.get(f'{prefix}:FREQUENCY:VALUE:X'), dtype=dtype, device=device)
error_nux = torch.tensor(cs.get(f'{prefix}:FREQUENCY:ERROR:X'), dtype=dtype, device=device)

# Load y frequency data
value_nuy = torch.tensor(cs.get(f'{prefix}:FREQUENCY:VALUE:Y'), dtype=dtype, device=device)
error_nuy = torch.tensor(cs.get(f'{prefix}:FREQUENCY:ERROR:Y'), dtype=dtype, device=device)

# Load x amplitude data
pv_value_ax = [f'{prefix}:{name}:AMPLITUDE:VALUE:X' for name in name]
pv_error_ax = [f'{prefix}:{name}:AMPLITUDE:ERROR:X' for name in name]
value_ax = factor*torch.tensor([cs.get(pv) for pv in pv_value_ax], dtype=dtype, device=device)
error_ax = factor*torch.tensor([cs.get(pv) for pv in pv_error_ax], dtype=dtype, device=device)

# Load y amplitude data
pv_value_ay = [f'{prefix}:{name}:AMPLITUDE:VALUE:Y' for name in name]
pv_error_ay = [f'{prefix}:{name}:AMPLITUDE:ERROR:Y' for name in name]
value_ay = factor*torch.tensor([cs.get(pv) for pv in pv_value_ay], dtype=dtype, device=device)
error_ay = factor*torch.tensor([cs.get(pv) for pv in pv_error_ay], dtype=dtype, device=device)

# Load x phase data
pv_value_fx = [f'{prefix}:{name}:PHASE:VALUE:X' for name in name]
pv_error_fx = [f'{prefix}:{name}:PHASE:ERROR:X' for name in name]
value_fx = torch.tensor([cs.get(pv) for pv in pv_value_fx], dtype=dtype, device=device)
error_fx = torch.tensor([cs.get(pv) for pv in pv_error_fx], dtype=dtype, device=device)

# Load y phase data
pv_value_fy = [f'{prefix}:{name}:PHASE:VALUE:Y' for name in name]
pv_error_fy = [f'{prefix}:{name}:PHASE:ERROR:Y' for name in name]
value_fy = torch.tensor([cs.get(pv) for pv in pv_value_fy], dtype=dtype, device=device)
error_fy = torch.tensor([cs.get(pv) for pv in pv_error_fy], dtype=dtype, device=device)
if args.verbose:
  print('PV list:')
  for pv in [*pv_value_ax, *pv_error_ax, *pv_value_ay, *pv_error_ay, *pv_value_fx, *pv_error_fx, *pv_value_fy, *pv_error_fy]:
    print(pv)

# Set model
model_path = args.model
if not os.path.isabs(model_path) and not os.path.exists(model_path):
  local = os.path.join(os.path.dirname(__file__), model_path)
  parent = os.path.join(os.path.dirname(__file__), '..', model_path)
  if os.path.exists(local):
    model_path = local
  elif os.path.exists(parent):
    model_path = parent
model = Model(path=model_path, dtype=dtype, device=device)
model.flag[model.monitor_index] = flag

# Set table
table = Table(name, value_nux, value_nuy, value_ax, value_ay, value_fx, value_fy,
              error_nux, error_nuy, error_ax, error_ay, error_fx, error_fy,
              dtype=dtype, device=device)

# Set twiss
twiss = Twiss(model, table, limit=args.limit)
if args.verbose:
  print(f'Twiss: {twiss}')

# Compute actions
twiss.get_action(dict_threshold={'use': args.clean, 'factor': args.factor})

# Compute twiss from amplitude
twiss.get_twiss_from_amplitude()

# Compute twiss from phase
twiss.phase_virtual()
twiss.get_twiss_from_phase()
mask_x = twiss.filter_twiss(plane='x')
mask_y = twiss.filter_twiss(plane='y')
if args.limit != 1:
  twiss.process_twiss(plane='x', mask=mask_x, weight=True)
  twiss.process_twiss(plane='y', mask=mask_y, weight=True)
else:
  twiss.process_twiss(plane='x')
  twiss.process_twiss(plane='y')

# Set model twiss
ax_m, bx_m = twiss.model.ax[1:-1].cpu().numpy(), twiss.model.bx[1:-1].cpu().numpy()
ay_m, by_m = twiss.model.ay[1:-1].cpu().numpy(), twiss.model.by[1:-1].cpu().numpy()
model_position = twiss.model.time[1:-1].cpu().numpy()

# Set model phase data
fx_m, fy_m = twiss.model.monitor_phase_x.cpu().numpy(), twiss.model.monitor_phase_y.cpu().numpy()

# Set twiss from amplitude
bx_m_a = twiss.model.bx[twiss.model.monitor_index].cpu().numpy()
by_m_a = twiss.model.by[twiss.model.monitor_index].cpu().numpy()
bx_a, sigma_bx_a = twiss.data_amplitude['bx'].cpu().numpy(), twiss.data_amplitude['sigma_bx'].cpu().numpy()
by_a, sigma_by_a = twiss.data_amplitude['by'].cpu().numpy(), twiss.data_amplitude['sigma_by'].cpu().numpy()

# Set phase advance data
fx, sigma_fx = twiss.table.phase_x.cpu().numpy(), twiss.table.sigma_x.cpu().numpy()
fy, sigma_fy = twiss.table.phase_y.cpu().numpy(), twiss.table.sigma_y.cpu().numpy()

# Set twiss from phase
ax, sigma_ax = twiss.ax[1:-1].cpu().numpy(), twiss.sigma_ax[1:-1].cpu().numpy()
bx, sigma_bx = twiss.bx[1:-1].cpu().numpy(), twiss.sigma_bx[1:-1].cpu().numpy()
ay, sigma_ay = twiss.ay[1:-1].cpu().numpy(), twiss.sigma_ay[1:-1].cpu().numpy()
by, sigma_by = twiss.by[1:-1].cpu().numpy(), twiss.sigma_by[1:-1].cpu().numpy()

# Plot
if args.plot:
  from pandas import DataFrame
  from plotly.subplots import make_subplots
  from plotly.express import scatter
  from plotly.express import line
  x_axis = 'POSITION' if args.position else 'BPM'
  x_label = 'POSITION' if args.position else 'BPM'
  if args.phase:
    df = DataFrame()
    df['BPM'] = name
    df['POSITION'] = position
    df['FX'] = fx
    df['FX_M'] = fx_m
    df['ERROR_FX'] = fx_m - fx
    df['SIGMA_FX'] = sigma_fx
    df['FY'] = fy
    df['FY_M'] = fy_m
    df['ERROR_FY'] = fy_m - fy
    df['SIGMA_FY'] = sigma_fy
    plot = make_subplots(rows=2, cols=2, vertical_spacing=0.1, shared_xaxes=True, horizontal_spacing=0.05, subplot_titles=(f'sum(abs(FX_M-FX))={sum(abs(fx_m - fx)):9.6}', f'sum(abs(FY_M-FY))={sum(abs(fy_m - fy)):9.6}', ' ', ' '))
    plot.update_layout(title_text=f'{TIME}: TWISS (PHASE): LIMIT={twiss.limit}')
    pltx1 = scatter(df, x=x_axis, y='FX', error_y='SIGMA_FX', color_discrete_sequence=['blue'], hover_data=['BPM', 'FX', 'SIGMA_FX'])
    pltx1.update_traces(marker={'size': 10})
    pltx1, *_ = pltx1.data
    plot.add_trace(pltx1, row=1, col=1)
    pltx1 = line(df, x=x_axis, y='FX_M', color_discrete_sequence=['black'], markers=True, hover_data=['BPM', 'FX_M'], symbol_sequence=['square-open'])
    pltx1.update_traces(marker={'size': 10})
    pltx1, *_ = pltx1.data
    plot.add_trace(pltx1, row=1, col=1)
    plot.update_xaxes(title_text=x_label, row=1, col=1)
    plot.update_yaxes(title_text='FX', row=1, col=1)
    plty1 = scatter(df, x=x_axis, y='FY', error_y='SIGMA_FY', color_discrete_sequence=['blue'], hover_data=['BPM', 'FY', 'SIGMA_FY'])
    plty1.update_traces(marker={'size': 10})
    plty1, *_ = plty1.data
    plot.add_trace(plty1, row=1, col=2)
    plty1 = line(df, x=x_axis, y='FY_M', color_discrete_sequence=['black'], markers=True, hover_data=['BPM', 'FY_M'], symbol_sequence=['square-open'])
    plty1.update_traces(marker={'size': 10})
    plty1, *_ = plty1.data
    plot.add_trace(plty1, row=1, col=2)
    plot.update_xaxes(title_text=x_label, row=1, col=2)
    plot.update_yaxes(title_text='FY', row=1, col=2)
    pltx2 = scatter(df, x=x_axis, y='ERROR_FX', error_y='SIGMA_FX', color_discrete_sequence=['blue'], hover_data=['BPM', 'ERROR_FX', 'SIGMA_FX'])
    pltx2.update_traces(marker={'size': 10})
    pltx2, *_ = pltx2.data
    plot.add_trace(pltx2, row=2, col=1)
    plot.update_xaxes(title_text=x_label, row=2, col=1)
    plot.update_yaxes(title_text='FX_M-FX', row=2, col=1)
    plty2 = scatter(df, x=x_axis, y='ERROR_FY', error_y='SIGMA_FY', color_discrete_sequence=['blue'], hover_data=['BPM', 'ERROR_FX', 'SIGMA_FY'])
    plty2.update_traces(marker={'size': 10})
    plty2, *_ = plty2.data
    plot.add_trace(plty2, row=2, col=2)
    plot.update_xaxes(title_text=x_label, row=2, col=2)
    plot.update_yaxes(title_text='FY_M-FY', row=2, col=2)
    config = {'toImageButtonOptions': {'height':None, 'width':None}, 'modeBarButtonsToRemove': ['lasso2d', 'select2d'], 'modeBarButtonsToAdd':['drawopenpath', 'eraseshape'], 'scrollZoom': True}
    plot.show(config=config)

  df = DataFrame()
  df['BPM'] = twiss.model.name[1:-1]
  df['POSITION'] = model_position
  df['AX'] = ax
  df['AX_M'] = ax_m
  df['SIGMA_AX'] = sigma_ax
  df['ERROR_AX'] = (ax_m - ax)
  df['BX'] = bx
  df['BX_M'] = bx_m
  df['SIGMA_BX'] = sigma_bx
  df['ERROR_BX'] = (bx_m - bx)/bx_m
  df['DELTA_BX'] = sigma_bx/bx_m
  df['AY'] = ay
  df['AY_M'] = ay_m
  df['SIGMA_AY'] = sigma_ay
  df['ERROR_AY'] = (ay_m - ay)
  df['BY'] = by
  df['BY_M'] = by_m
  df['SIGMA_BY'] = sigma_by
  df['ERROR_BY'] = (by_m - by)/by_m
  df['DELTA_BY'] = sigma_by/by_m
  monitor_index = numpy.array([index - 1 for index in model.monitor_index], dtype=numpy.int64)
  virtual_index = numpy.array([index - 1 for index in model.virtual_index if 0 < index < model.size - 1], dtype=numpy.int64)
  if monitor_index.size == 0:
    rms_bx = 100.0*numpy.sqrt(numpy.nanmean(df['ERROR_BX'].to_numpy()**2))
    rms_by = 100.0*numpy.sqrt(numpy.nanmean(df['ERROR_BY'].to_numpy()**2))
  else:
    rms_bx = 100.0*numpy.sqrt(numpy.nanmean(df['ERROR_BX'].to_numpy()[monitor_index]**2))
    rms_by = 100.0*numpy.sqrt(numpy.nanmean(df['ERROR_BY'].to_numpy()[monitor_index]**2))
  plot = make_subplots(rows=2, cols=2, vertical_spacing=0.1, shared_xaxes=True, horizontal_spacing=0.05, subplot_titles=(f'RMS X = {rms_bx:9.6f} %', f'RMS Y = {rms_by:9.6f} %', ' ', ' '))
  plot.update_layout(title_text=f'{TIME}: TWISS (PHASE): LIMIT={twiss.limit}')
  pltx1 = scatter(df, x=x_axis, y='BX', error_y='SIGMA_BX', color_discrete_sequence=['blue'], hover_data=['BPM', 'BX', 'SIGMA_BX'], symbol_sequence=['circle-open'])
  pltx1.update_traces(marker={'size': 10})
  pltx1.update_traces(name='BX', legendgroup='1', showlegend=True)
  pltx1, *_ = pltx1.data
  plot.add_trace(pltx1, row=1, col=1)
  pltx1 = line(df, x=x_axis, y='BX_M', color_discrete_sequence=['black'], markers=True, hover_data=['BPM', 'BX_M'], symbol_sequence=['square-open'])
  pltx1.update_traces(marker={'size': 10})
  pltx1.update_traces(name='BX_M', legendgroup='2', showlegend=True)
  pltx1, *_ = pltx1.data
  plot.add_trace(pltx1, row=1, col=1)
  plot.update_xaxes(title_text=x_label, row=1, col=1)
  plot.update_yaxes(title_text='BX', row=1, col=1)
  plty1 = scatter(df, x=x_axis, y='BY', error_y='SIGMA_BY', color_discrete_sequence=['blue'], hover_data=['BPM', 'BY', 'SIGMA_BY'], symbol_sequence=['circle-open'])
  plty1.update_traces(marker={'size': 10})
  plty1.update_traces(name='BY', legendgroup='3', showlegend=True)
  plty1, *_ = plty1.data
  plot.add_trace(plty1, row=1, col=2)
  plty1 = line(df, x=x_axis, y='BY_M', color_discrete_sequence=['black'], markers=True, hover_data=['BPM', 'BY_M'], symbol_sequence=['square-open'])
  plty1.update_traces(marker={'size': 10})
  plty1.update_traces(name='BY_M', legendgroup='4', showlegend=True)
  plty1, *_ = plty1.data
  plot.add_trace(plty1, row=1, col=2)
  plot.update_xaxes(title_text=x_label, row=1, col=2)
  plot.update_yaxes(title_text='BY', row=1, col=2)
  pltx2 = scatter(df, x=x_axis, y='ERROR_BX', error_y='DELTA_BX', color_discrete_sequence=['blue'], hover_data=['BPM', 'ERROR_BX', 'DELTA_BX'], symbol_sequence=['circle-open'])
  pltx2.update_traces(marker={'size': 10})
  pltx2.update_traces(name='(BX_M-BX)/BX_M', legendgroup='5', showlegend=True)
  pltx2, *_ = pltx2.data
  plot.add_trace(pltx2, row=2, col=1)
  plot.update_xaxes(title_text=x_label, row=2, col=1)
  plot.update_yaxes(title_text='(BX_M-BX)/BX_M', row=2, col=1)
  plty2 = scatter(df, x=x_axis, y='ERROR_BY', error_y='DELTA_BY', color_discrete_sequence=['blue'], hover_data=['BPM', 'ERROR_BY', 'DELTA_BY'], symbol_sequence=['circle-open'])
  plty2.update_traces(marker={'size': 10})
  plty2.update_traces(name='(BY_M-BY)/BY_M', legendgroup='6', showlegend=True)
  plty2, *_ = plty2.data
  plot.add_trace(plty2, row=2, col=2)
  plot.update_xaxes(title_text=x_label, row=2, col=2)
  plot.update_yaxes(title_text='(BY_M-BY)/BY_M', row=2, col=2)
  if virtual_index.size != 0:
    virtual = df.iloc[virtual_index]
    trace = scatter(virtual, x=x_axis, y='BX', error_y='SIGMA_BX', color_discrete_sequence=['green'], hover_data=['BPM', 'BX', 'SIGMA_BX'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='BX (VIRTUAL)', legendgroup='virtual', showlegend=True)
    virtual, *_ = trace.data
    plot.add_trace(virtual, row=1, col=1)

    virtual = df.iloc[virtual_index]
    trace = scatter(virtual, x=x_axis, y='BY', error_y='SIGMA_BY', color_discrete_sequence=['green'], hover_data=['BPM', 'BY', 'SIGMA_BY'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='BY (VIRTUAL)', legendgroup='virtual', showlegend=False)
    virtual, *_ = trace.data
    plot.add_trace(virtual, row=1, col=2)

    virtual = df.iloc[virtual_index]
    trace = scatter(virtual, x=x_axis, y='ERROR_BX', error_y='DELTA_BX', color_discrete_sequence=['green'], hover_data=['BPM', 'ERROR_BX', 'DELTA_BX'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='(BX_M-BX)/BX_M (VIRTUAL)', legendgroup='virtual', showlegend=False)
    virtual, *_ = trace.data
    plot.add_trace(virtual, row=2, col=1)

    virtual = df.iloc[virtual_index]
    virtuatracel = scatter(virtual, x=x_axis, y='ERROR_BY', error_y='DELTA_BY', color_discrete_sequence=['green'], hover_data=['BPM', 'ERROR_BY', 'DELTA_BY'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='(BY_M-BY)/BY_M (VIRTUAL)', legendgroup='virtual', showlegend=False)
    virtual, *_ = trace.data
    plot.add_trace(virtual, row=2, col=2)
  if args.amplitude:
    mask = DataFrame({'BPM':name, 'POSITION':position, 'BX':bx_a, 'SIGMA_BX':sigma_bx_a})
    trace = scatter(mask, x=x_axis, y='BX', error_y='SIGMA_BX', color_discrete_sequence=['red'], hover_data=['BPM', 'BX', 'SIGMA_BX'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='BX_A', legendgroup='7', showlegend=True)
    mask, *_ = trace.data
    plot.add_trace(mask, row=1, col=1)
    mask = DataFrame({'BPM':name, 'POSITION':position, 'BY':by_a, 'SIGMA_BY':sigma_by_a})
    trace = scatter(mask, x=x_axis, y='BY', error_y='SIGMA_BY', color_discrete_sequence=['red'], hover_data=['BPM', 'BY', 'SIGMA_BY'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='BY_A', legendgroup='8', showlegend=True)
    mask, *_ = trace.data
    plot.add_trace(mask, row=1, col=2)
    mask = DataFrame({'BPM':name, 'POSITION':position, 'ERROR_BX':((bx_m_a) - bx_a)/(bx_m_a), 'DELTA_BX':sigma_bx_a/(bx_m_a)})
    trace = scatter(mask, x=x_axis, y='ERROR_BX', error_y='DELTA_BX', color_discrete_sequence=['red'], hover_data=['BPM', 'ERROR_BX', 'DELTA_BX'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='(BX_M-BX_A)/BX_M', legendgroup='9', showlegend=True)
    mask, *_ = trace.data
    plot.add_trace(mask, row=2, col=1)
    mask = DataFrame({'BPM':name, 'POSITION':position, 'ERROR_BY':((by_m_a) - by_a)/(by_m_a), 'DELTA_BY':sigma_by_a/(by_m_a)})
    trace = scatter(mask, x=x_axis, y='ERROR_BY', error_y='DELTA_BY', color_discrete_sequence=['red'], hover_data=['BPM', 'ERROR_BY', 'DELTA_BY'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='(BY_M-BY_A)/BY_M', legendgroup='10', showlegend=True)
    mask, *_ = trace.data
    plot.add_trace(mask, row=2, col=2)
  plot.update_layout(legend=dict(orientation='h', yanchor='bottom', y=-0.2, xanchor='center', x=0.5))
  config = {'toImageButtonOptions': {'height':None, 'width':None}, 'modeBarButtonsToRemove': ['lasso2d', 'select2d'], 'modeBarButtonsToAdd':['drawopenpath', 'eraseshape'], 'scrollZoom': True}
  plot.show(config=config)

  plot = make_subplots(rows=2, cols=2, vertical_spacing=0.1, shared_xaxes=True, horizontal_spacing=0.05, subplot_titles=(f'sum(abs(AX_M-AX))={sum(abs(ax_m - ax)):9.6}', f'sum(abs(AY_M-AY))={sum(abs(ay_m - ay)):9.6}', ' ', ' '))
  plot.update_layout(title_text=f'{TIME}: TWISS (PHASE): LIMIT={twiss.limit}')
  pltx1 = scatter(df, x=x_axis, y='AX', error_y='SIGMA_AX', color_discrete_sequence=['blue'], hover_data=['BPM', 'AX', 'SIGMA_AX'], symbol_sequence=['circle-open'])
  pltx1.update_traces(marker={'size': 10})
  pltx1.update_traces(name='AX', legendgroup='11', showlegend=True)
  pltx1, *_ = pltx1.data
  plot.add_trace(pltx1, row=1, col=1)
  pltx1 = line(df, x=x_axis, y='AX_M', color_discrete_sequence=['black'], markers=True, hover_data=['BPM', 'AX_M'], symbol_sequence=['square-open'])
  pltx1.update_traces(marker={'size': 10})
  pltx1.update_traces(name='AX_M', legendgroup='12', showlegend=True)
  pltx1, *_ = pltx1.data
  plot.add_trace(pltx1, row=1, col=1)
  plot.update_xaxes(title_text=x_label, row=1, col=1)
  plot.update_yaxes(title_text='AX', row=1, col=1)
  plty1 = scatter(df, x=x_axis, y='AY', error_y='SIGMA_AY', color_discrete_sequence=['blue'], hover_data=['BPM', 'AY', 'SIGMA_AY'], symbol_sequence=['circle-open'])
  plty1.update_traces(marker={'size': 10})
  plty1.update_traces(name='AY', legendgroup='13', showlegend=True)
  plty1, *_ = plty1.data
  plot.add_trace(plty1, row=1, col=2)
  plty1 = line(df, x=x_axis, y='AY_M', color_discrete_sequence=['black'], markers=True, hover_data=['BPM', 'AY_M'], symbol_sequence=['square-open'])
  plty1.update_traces(marker={'size': 10})
  plty1.update_traces(name='AY_M', legendgroup='14', showlegend=True)
  plty1, *_ = plty1.data
  plot.add_trace(plty1, row=1, col=2)
  plot.update_xaxes(title_text=x_label, row=1, col=2)
  plot.update_yaxes(title_text='AY', row=1, col=2)
  pltx2 = scatter(df, x=x_axis, y='ERROR_AX', error_y='SIGMA_AX', color_discrete_sequence=['blue'], hover_data=['BPM', 'ERROR_AX', 'SIGMA_BX'], symbol_sequence=['circle-open'])
  pltx2.update_traces(marker={'size': 10})
  pltx2, *_ = pltx2.data
  plot.add_trace(pltx2, row=2, col=1)
  plot.update_xaxes(title_text=x_label, row=2, col=1)
  plot.update_yaxes(title_text='AX_M-BX', row=2, col=1)
  plty2 = scatter(df, x=x_axis, y='ERROR_AY', error_y='SIGMA_AY', color_discrete_sequence=['blue'], hover_data=['BPM', 'ERROR_AY', 'SIGMA_BY'], symbol_sequence=['circle-open'])
  plty2.update_traces(marker={'size': 10})
  plty2, *_ = plty2.data
  plot.add_trace(plty2, row=2, col=2)
  plot.update_xaxes(title_text=x_label, row=2, col=2)
  plot.update_yaxes(title_text='AY_M-AY', row=2, col=2)
  if virtual_index.size != 0:
    virtual = df.iloc[virtual_index]
    trace = scatter(virtual, x=x_axis, y='AX', error_y='SIGMA_AX', color_discrete_sequence=['green'], hover_data=['BPM', 'AX', 'SIGMA_AX'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='AX (VIRTUAL)', legendgroup='virtual', showlegend=True)
    virtual, *_ = trace.data
    plot.add_trace(virtual, row=1, col=1)

    virtual = df.iloc[virtual_index]
    trace = scatter(virtual, x=x_axis, y='AY', error_y='SIGMA_AY', color_discrete_sequence=['green'], hover_data=['BPM', 'AY', 'SIGMA_AY'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='AY (VIRTUAL)', legendgroup='virtual', showlegend=False)
    virtual, *_ = trace.data
    plot.add_trace(virtual, row=1, col=2)

    virtual = df.iloc[virtual_index]
    trace = scatter(virtual, x=x_axis, y='ERROR_AX', error_y='SIGMA_AX', color_discrete_sequence=['green'], hover_data=['BPM', 'ERROR_AX', 'SIGMA_AX'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='AX_M-AX (VIRTUAL)', legendgroup='virtual', showlegend=False)
    virtual, *_ = trace.data
    plot.add_trace(virtual, row=2, col=1)

    virtual = df.iloc[virtual_index]
    trace = scatter(virtual, x=x_axis, y='ERROR_AY', error_y='SIGMA_AY', color_discrete_sequence=['green'], hover_data=['BPM', 'ERROR_AY', 'SIGMA_AY'], symbol_sequence=['circle-open'])
    trace.update_traces(marker={'size': 10})
    trace.update_traces(name='AY_M-AY (VIRTUAL)', legendgroup='virtual', showlegend=False)
    virtual, *_ = trace.data
    plot.add_trace(virtual, row=2, col=2)
  plot.update_layout(legend=dict(orientation='h', yanchor='bottom', y=-0.2, xanchor='center', x=0.5))
  config = {'toImageButtonOptions': {'height':None, 'width':None}, 'modeBarButtonsToRemove': ['lasso2d', 'select2d'], 'modeBarButtonsToAdd':['drawopenpath', 'eraseshape'], 'scrollZoom': True}
  plot.show(config=config)

# Save to cs
if args.update:
  for bpm, value, error in zip(name, bx_a, sigma_bx_a):
    cs.set(f'{args.prefix}:{bpm}:AMPLITUDE:BX:VALUE', value)
    cs.set(f'{args.prefix}:{bpm}:AMPLITUDE:BX:ERROR', error)
  for bpm, value, error in zip(name, by_a, sigma_by_a):
    cs.set(f'{args.prefix}:{bpm}:AMPLITUDE:BY:VALUE', value)
    cs.set(f'{args.prefix}:{bpm}:AMPLITUDE:BY:ERROR', error)
  for bpm, value, error in zip(model.name[1:-1], bx, sigma_bx):
    cs.set(f'{args.prefix}:{bpm}:PHASE:BX:VALUE', value)
    cs.set(f'{args.prefix}:{bpm}:PHASE:BX:ERROR', error)
  for bpm, value, error in zip(model.name[1:-1], by, sigma_by):
    cs.set(f'{args.prefix}:{bpm}:PHASE:BY:VALUE', value)
    cs.set(f'{args.prefix}:{bpm}:PHASE:BY:ERROR', error)
  for bpm, value, error in zip(model.name[1:-1], ax, sigma_ax):
    cs.set(f'{args.prefix}:{bpm}:PHASE:AX:VALUE', value)
    cs.set(f'{args.prefix}:{bpm}:PHASE:AX:ERROR', error)
  for bpm, value, error in zip(model.name[1:-1], ay, sigma_ay):
    cs.set(f'{args.prefix}:{bpm}:PHASE:AY:VALUE', value)
    cs.set(f'{args.prefix}:{bpm}:PHASE:AY:ERROR', error)
