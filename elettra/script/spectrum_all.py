...
parser.add_argument('--f_min', type=float, help='min frequency value for joined spectrum', default=None)
parser.add_argument('--f_max', type=float, help='max frequency value for joined spectrum', default=None)
...
if (args.f_min is None) ^ (args.f_max is None):
  exit('error: --f_min and --f_max should be used together')
if args.f_min is not None and args.f_min < 0.0:
  exit(f'error: {args.f_min=}, should be positive')
if args.f_max is not None and args.f_max < args.f_min:
  exit(f'error: {args.f_max=} should be greater than {args.f_min=}')
