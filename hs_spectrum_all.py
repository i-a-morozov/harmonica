"""
usage: hs_spectrum_all [-h] [-p {x,z,i}] [-l LENGTH] [-s BPM [BPM ...] | -o BPM [BPM ...]] [-r] [--beta_min BETA_MIN] [--beta_max BETA_MAX] [-f] [-c | -n] [--print]
                       [--normalize] [-w] [--name {cosine_window,kaiser_window}] [--order ORDER] [--min MIN] [--max MAX] [--nufft] [--log] [--plot] [--device {cpu,cuda}]
                       [--dtype {float32,float64}] [--test]
...
if args.plot:
    ...
    plot = scatter(x=grid, y=data, title=TIME, labels={'x': 'frequency', 'y': 'amplitude'})
    plot.show()
