...
usage: hs_frequency_all [-h] -p {x,z} [-l LENGTH] [--load LOAD] [--shift SHIFT] [--skip BPM [BPM ...] | --only BPM [BPM ...]] [-o OFFSET] [-r] [-f] [-c | -n] [--print]
                        ...
                        [--beta_max BETA_MAX] [--nufft] [--time {position,phase}] [--plot] [--device {cpu,cuda}] [--dtype {float32,float64}] [--test]
...
if args.file and args.numpy:
  numpy.save(f'frequency_all_{TIME}.npy', frequency)
if args.file and args.csv:
  numpy.savetxt(f'frequency_all_{TIME}.csv', frequency, delimiter=',')