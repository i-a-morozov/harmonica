...
class Wrapper():
    ...
    Note, if the original objective is differentible, so will be the wrapped objective
    ...
    Wrapped objective must be first evaluated with None input or without parameters to attach attributes and methods
    ...
    __repr__(self) -> str
        String representation (attached)
    ...
    __call__(self, objective:Objetive, *, wrapped:bool=True) -> Objetive
    ...
    def __repr__(self) -> str:
        ...
        Note, data type and device are excluded
    ...
    def __call__(self,
        ...
        Evaluate output with input knobs (unit cube) to return (value, error) tuple
        ...
        *args:
        ...
        def wrapper(knobs:Optional[Knobs]=None, *args) -> tuple[Value, Error]:
            ...
            if knobs is None:
                ...
                if wrapper.l2 is not None: value += wrapper.l2*(knobs**2).sum()
