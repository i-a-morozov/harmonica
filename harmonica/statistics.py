"""
Statistics module.
Compute different statistics for 1D or 2D data.
Standardize and rescale 1D or 2D data.
...
def standard_deviation(data:torch.Tensor) -> torch.Tensor:
    """
    ...
    Returns
    -------
    standard deviation (torch.Tensor)

    """
    return variance(data).sqrt()


def root_mean_square(data:torch.Tensor) -> torch.Tensor:
    """
    ...
    Returns
    -------
    root mean square (torch.Tensor)

    """
    return mean(data**2).sqrt()


def weighted_mean(data:torch.Tensor,
...
def weighted_variance(data:torch.Tensor,
...
def trimmed_mean(data:torch.Tensor,
                 *,
                 ...
                 max_fraction:float=0.1) -> torch.Tensor:
    """
    ...
    """
    *_, size = data.shape
    min_drop, max_drop = int(min_fraction*size), int(size - max_fraction*size)
...
def winsorized_mean(data:torch.Tensor,
    ...
    *_, size = data.shape
    ...
    min_value, max_value = data[:, min_drop], data[:, max_drop]
...
def median(data:torch.Tensor,
    ...
    """
    return data.quantile(0.5, -1, **kwargs)
...
def biweight_midvariance(data:torch.Tensor,
                         scale:torch.Tensor=10.0,
                         *,
                         center:torch.Tensor=None,
                         deviation:torch.Tensor=None,
                         **kwargs) -> torch.Tensor:
    """
...
def quantile(data:torch.Tensor,
    ...
    """
    return torch.quantile(data, fraction, -1, **kwargs).swapaxes(0, -1)


def quartiles(data:torch.Tensor,
              **kwargs) -> torch.Tensor:
    """
...
def interquartile_range(data:torch.Tensor,
                        **kwargs) -> torch.Tensor:
    """
    ...
    """
    q_min, _, q_max = quartiles(data, **kwargs).swapaxes(0, -1)
    return (q_max - q_min)
...
def standardize(data:torch.Tensor,
                *,
                center_estimator=mean,
                spread_estimator=variance) -> torch.Tensor:
    """
...
def rescale(data:torch.Tensor,
            *,
            ...
            scale_max:float=None) -> torch.Tensor:
    """
